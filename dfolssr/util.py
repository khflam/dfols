...
def eval_least_squares_with_regularisation(objfun, x, h=None, argsf=(), argsh=(), verbose=True, eval_num=0, pt_num=0, full_x_thresh=6, check_for_overflow=True):
    # Evaluate least squares function (+ regularisation function h if provided). This function is only used for monitoring performance. 
    # It is not used for construction of the fully linear model that approximates the black box function f.
    fvec = objfun(x, *argsf)
...
def calculate_u(H, L_h=None, func_tol=1e-3):
        ...
        u = 2 * func_tol / (L_h ** 2 + L_h * np.sqrt(L_h ** 2 + 2 * L_grad_m * func_tol))
        return u


def moreau_envelope(xopt=(), h=None, argsh=(), u=None, prox_uh=None, argsprox=()):
    # Calculate Moreau envelope of h,
    # M_h_u(x) = h(prox_uh(xopt, u)) + 0.5 * sumsq(xopt - prox_uh(xopt, u)) / u
    assert(all([h, u, prox_uh]))
    prox_uh_value = prox_uh(xopt, u, *argsprox)
    return h(prox_uh_value, *argsh) + 0.5 * sumsq(xopt - prox_uh_value) / u


def model_value(g, H, s, xopt=(), h=None, argsh=(), u=None, prox_uh=None, argsprox=()):
    ...
    if h != None:
        hvalue = moreau_envelope(xopt+s, h, argsh, u, prox_uh, argsprox)
        rtn += hvalue
    return rtn
