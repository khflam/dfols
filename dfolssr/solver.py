...
def solve_main(objfun, x0, argsf, xl, xu, projections, npt, rhobeg, rhoend, maxfun, nruns_so_far, nf_so_far, nx_so_far, nsamples, params,
               diagnostic_info, scaling_changes, h=None, argsh=(), L_h=None, prox_uh=None, argsprox=None, func_tol_init=None, func_tol_dec=None, func_tol_lb=None,
               r0_avg_old=None, r0_nsamples_old=None, default_growing_method_set_by_user=None, do_logging=True, print_progress=False):
    func_tol = func_tol_init
    ...
    control = Controller(objfun, argsf, x0, r0_avg, num_samples_run, xl, xu, projections, npt, rhobeg, rhoend, nf, nx, maxfun,
                         params, scaling_changes, do_logging, h, argsh, L_h, prox_uh, argsprox, func_tol_init)
    ...
    # case 1: no regularisation (h is None)
    # case 2: regularisation (h != None with u and u_tol positive parameters with u > u_tol)
    if h is None: # case 1: h is not provided
        while True: 
            current_iter += 1
            ...
            if dnorm < params("general.safety_step_thresh") * control.rho and not finished_growing and params("growing.safety.do_safety_step"):
                ...
                # question for Lindon: what should I do here? 
                # previously it was control.delta <= 1.5 control.rho but unittest would fail for test_solver.TestInverseProblem
                # should this logic be moved to lines 560 and 568? (this would match what's in DFOLS paper)
                if control.delta <= control.rho:  # cap trust region radius at rho 
            ...
            # (end main loop)
        # end of while loop for h is None case
    else: # case 2: h is provided
        while func_tol > func_tol_lb:
            while True:
                current_iter += 1
                ...
                # Trust region step
                d, gopt, H, gnew, crvmin = control.trust_region_step(params)
                u = calculate_u(H, L_h, control.func_tol)
                if do_logging:
                    module_logger.debug("Trust region step is d = " + str(d))
                xnew = control.model.xopt() + d
                dnorm = min(LA.norm(d), control.delta)
                ...
                if dnorm < params("general.safety_step_thresh") * control.rho and not finished_growing and params("growing.safety.do_safety_step"):
                    ...
                    # If we are done with the current rho, or didn't fix geometry above, reduce rho
                    if control.rho > rhoend: # previously if control.rho >  rhoend
                        # Reduce rho
                        control.reduce_rho(current_iter, params)
                        if do_logging:
                            module_logger.info("New rho = %g after %i function evaluations" % (control.rho, control.nf))
                        ...
                        if params("restarts.use_restarts") and params("restarts.use_soft_restarts"):
                            ...
                            rvec_list, obj_list, num_samples_run, exit_info = control.evaluate_objective(x, number_of_samples,
                                                                                                    params)
                            obj_new = sumsq(np.mean(rvec_list[:num_samples_run, :], axis=0)) + moreau_envelope(x, h, argsh, u, prox_uh, argsprox)
                            
                            if num_samples_run > 0:
                                    control.model.save_point(x, np.mean(rvec_list[:num_samples_run, :], axis=0),
                                                            num_samples_run, x_in_abs_coords=True)
                                    # control.save_approx_point(x, np.mean(rvec_list[:num_samples_run, :], axis=0), obj_new, num_samples_run)
                    ...
                    # early termination for small delta
                    if control.delta < min(u ** 2, 1e-16):
                        break
                    ...
                    if ratio > 0.0:
                        ...
                        # Update list of successful steps
                        this_step_was_not_improvement = control.model.est_objsave is not None and control.model.est_objopt() > control.model.est_objsave
                        succ_steps_not_improvement.pop()  # remove last item
                        succ_steps_not_improvement.insert(0, this_step_was_not_improvement)  # add at beginning
                        # Terminate (not restart) if all are True
                        if all(succ_steps_not_improvement):
...
def solve(objfun, x0, h=None, argsh=(), L_h=None, prox_uh=None, argsprox=(), func_tol_init=1e-2, func_tol_dec=1e-1, func_tol_lb=1e-5,
          argsf=(), bounds=None, projections=[], npt=None, rhobeg=None, rhoend=1e-8, maxfun=None, nsamples=None, user_params=None,
    ...
    if exit_info is None and h != None and (L_h == None or L_h <= 0.0):
        exit_info = ExitInformation(EXIT_INPUT_ERROR, "L_h is not None and must be positive")
    ...
    xmin, rmin, objmin, jacmin, nsamples_min, nf, nx, nruns, exit_info, diagnostic_info = \
        solve_main(objfun, x0, argsf, xl, xu, projections, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
                    diagnostic_info, scaling_changes, h, argsh, L_h, prox_uh, argsprox, func_tol_init, func_tol_dec, func_tol_lb,
                    default_growing_method_set_by_user=default_growing_method_set_by_user, do_logging=do_logging, print_progress=print_progress)
        ...
        if params("restarts.hard.use_old_rk"):
            xmin2, rmin2, objmin2, jacmin2, nsamples2, nf, nx, nruns, exit_info, diagnostic_info = \
                solve_main(objfun, xmin, argsf, xl, xu, projections, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
                            diagnostic_info, scaling_changes, h, argsh, L_h, prox_uh, argsprox, func_tol_init, func_tol_dec, func_tol_lb,
                            r0_avg_old=rmin, r0_nsamples_old=nsamples_min, do_logging=do_logging, print_progress=print_progress)
        else:
            xmin2, rmin2, objmin2, jacmin2, nsamples2, nf, nx, nruns, exit_info, diagnostic_info = \
                solve_main(objfun, xmin, argsf, xl, xu, projections, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
                           diagnostic_info, scaling_changes, h, argsh, L_h, prox_uh, argsprox, func_tol_init, func_tol_dec, func_tol_lb,
