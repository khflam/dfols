...
def ctrsbox_sfista(xopt, g, H, projections, delta, h=None, argsh=(), L_h=None, prox_uh=None, argsprox=(), func_tol=1e-3,
    ...
    # Estimate Lipschitz constant of least squares model
    # Initial guess of L is norm(Hessian)
    H_norm = np.linalg.norm(H, 2) # SOLVED: use ||H||_2 <= ||H||_F, might be better than maxhessian
    g_norm = np.linalg.norm(g, 2)
    # Lipschitz constant of quadratic is 2*||H||_2*delta
    # Lipschitz constant of linear term is just ||g||_2
    # L_m = 2 * delta * H_norm + g_norm
    # H_inv_g_norm = np.matmul(np.linalg.inv(H + tikhonov_reg*np.identity(n)), g)
    ...
    # SOLVED: bug here, previous: g + H @ d + (d - prox_uh(xopt, u, d, *argsprox)) / u
        # print("max element of g")
        # print(g.max())
        # print("max element of H")
        # print(H.max())
        # print("max element of numerator of gradient of moreau envelope")
        # print((xopt + d - prox_uh(xopt + d, u, *argsprox)).max())
        # print("max element of gradient of moreau envelope")
        # print(((xopt + d - prox_uh(xopt + d, u, *argsprox)) / u).max())
        return g + H @ d + (xopt + d - prox_uh(xopt + d, u, *argsprox)) / u
    
    # Set Lipschitz constant of gradient of F_u = 
    L_Fu = L_grad_m + 1 / u

    # Calculate iterations for Cauchy decrease
    # MAX_LOOP_ITERS = 100 * n ** 2
    # Cauchy decrease given by ||\nabla m_k(0)||^2 / L
    # Note the below d is currently the zero vector
    # curr_grad = gradient_Fu(xopt, g, H, u, prox_uh, d, argsprox)
    # cauchy_dec = (np.linalg.norm(curr_grad, 2) ** 2) / L # upper bound of cauchy decrease
    # MAX_LOOP_ITERS = ceil(0.5 * cauchy_dec/ (L * delta ** 2))
    ...
    for ii in range(MAX_LOOP_ITERS):
        ...
        # if model_value(g, H, d, xopt, h, u, prox_uh, *argsh, *argsprox) > model_value(g, H, prev_d, xopt, h, u, prox_uh, *argsh, *argsprox):
        #     d = prev_d
