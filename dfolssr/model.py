...
from .util import dykstra, sumsq, calculate_u, moreau_envelope
...
class Model(object):
    def __init__(self, npt, x0, r0, xl, xu, projections, r0_nsamples, n=None, m=None, abs_tol=1e-12, rel_tol=1e-20, 
                 h=None, argsh=(), L_h=None, prox_uh=None, argsprox=(), func_tol=1e-3, svd_max_jac_cond=1e8,
                 precondition=True, do_logging=True):
        if n is None:
            n = len(x0)
        if m is None:
            m = len(r0)
        ...
        self.prox_uh = prox_uh
        self.argsprox = argsprox

        self.func_tol = func_tol
        self.svd_max_jac_cond = svd_max_jac_cond
        self.u = calculate_u([svd_max_jac_cond], L_h=L_h, func_tol=func_tol)
        ...
        self.est_objval = np.inf * np.ones((npt, ))  # overall objective value for each xpt
        self.objval[0] = sumsq(r0)
        self.est_objval[0] = sumsq(r0)
        if h != None:
            self.objval[0] += h(x0, *argsh)
            self.est_objval[0] += moreau_envelope(x0, h=h, argsh=argsh, u=self.u, prox_uh=prox_uh, argsprox=argsprox)
    ...
    def est_objopt(self):
        return self.est_objval[self.kopt]
    ...
    def est_objval(self, k):
        assert 0 <= k < self.npt(), "Invalid index %g" % k
        return self.est_objval[k]
    ...
    def change_point(self, k, x, rvec, allow_kopt_update=True):
        ...
        self.est_objval[k] = sumsq(rvec)
        if self.h != None:
            self.objval[k] += self.h(self.xbase + x, *self.argsh)
            self.est_objval[k] += moreau_envelope(self.xbase + x, h=self.h, argsh=self.argsh, u=self.u, prox_uh=self.prox_uh, argsprox=self.argsprox)
        self.nsamples[k] = 1
        self.factorisation_current = False

        if self.h != None and allow_kopt_update and self.est_objval[k] < self.est_objopt():
            self.kopt = k
        elif self.h == None and allow_kopt_update and self.objval[k] < self.objopt():
            self.kopt = k
        return
    ...
    def swap_points(self, k1, k2):
        ...
        self.est_objval[[k1, k2]] = self.est_objval[[k2, k1]]
        if self.kopt == k1:
            self.kopt = k2
        elif self.kopt == k2:
            self.kopt = k1
    ...
    def add_new_sample(self, k, rvec_extra):
        ...
        self.est_objval[k] = sumsq(self.fval_v[k, :])
        if self.h != None:
            self.objval[k] += self.h(self.xbase + self.points[k, :], *self.argsh)
            self.est_objval[k] += moreau_envelope(self.xbase + self.points[k, :], h=self.h, argsh=self.argsh, u=self.u, prox_uh=self.prox_uh, argsprox=self.argsprox)
        self.nsamples[k] += 1

        if self.h != None:
            self.kopt = np.argmin(self.est_objval[:self.npt()])
        else:
            self.kopt = np.argmin(self.objval[:self.npt()])  # make sure kopt is always the best value we have # here
        return
    ...
    def add_new_point(self, x, rvec):
        ...
        obj = sumsq(rvec)
        if self.h != None:
            objval_new = obj + self.h(self.xbase + x, *self.argsh)
            est_objval_new = obj + moreau_envelope(self.xbase + x, h=self.h, argsh=self.argsh, u=self.u, prox_uh=self.prox_uh, argsprox=self.argsprox)
        self.objval = np.append(self.objval, objval_new)  # append entry to objval
        self.est_objval = np.append(self.est_objval, est_objval_new)  # append entry to est_objval
        self.nsamples = np.append(self.nsamples, 1)  # add new sample number
        self.num_pts += 1  # make sure npt is updated
        self.npt_so_far += 1

        if self.h != None and est_objval_new < self.est_objopt():
            self.kopt = self.npt() - 1
        elif self.h == None and objval_new < self.objopt():
            self.kopt = self.npt() - 1
    ...
    def save_point(self, x, rvec, nsamples, x_in_abs_coords=True):
        ...
        obj = sumsq(rvec)
        if self.h != None:
            objval_new = obj + self.h(xabs, *self.argsh)
            est_objval_new = obj + moreau_envelope(xabs, h=self.h, argsh=self.argsh, u=self.u, prox_uh=self.prox_uh, argsprox=self.argsprox)
        
        if self.h != None and (self.est_objsave is None or obj <= self.est_objsave):
            self.xsave = xabs
            self.rsave = rvec.copy()
            self.objsave = objval_new
            self.est_objsave = est_objval_new
            self.jacsave = self.model_jac.copy()
            self.nsamples_save = nsamples
            return True
        elif self.h == None and (self.objsave is None or obj <= self.objsave):
            self.xsave = xabs
            self.rsave = rvec.copy()
            self.objsave = objval_new
            self.jacsave = self.model_jac.copy()
    ...
    def reset_model(self, func_tol_new):
        u_new = calculate_u([self.svd_max_jac_cond], L_h=self.L_h, func_tol=func_tol_new)
        for k in range(self.npt()):
            self.est_objval[k] = sumsq(self.fval_v[k, :]) + moreau_envelope(self.xpt(k,abs_coordinates=True), self.h, self.argsh, u_new, self.prox_uh, self.argsprox)
        self.func_tol = func_tol_new
        self.u = u_new
        
        self.kopt = np.argmin(self.est_objval[:self.npt()])
        self.objbeg = self.est_objopt() # reset objbeg for model when starting new DFOLS loop
