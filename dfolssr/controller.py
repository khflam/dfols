...
class Controller(object):
    def __init__(self, objfun, argsf, x0, r0, r0_nsamples, xl, xu, projections, npt, rhobeg, rhoend, nf, nx, maxfun, params,
                 scaling_changes, do_logging, h=None, argsh=(), L_h=None, prox_uh=None, argsprox=(), func_tol=1e-3):
        ...
        self.model = Model(npt, x0, r0, xl, xu, projections, r0_nsamples, precondition=params("interpolation.precondition"),
                           ...
                           h=h, argsh=argsh, L_h=L_h, prox_uh=prox_uh, argsprox=argsprox,
                           func_tol=func_tol, svd_max_jac_cond=params("growing.full_rank.svd_max_jac_cond"),
        ...
        if self.h != None:
            # objective of last successful run can be initialised as f+h to be a first guess
            # since precise calculate of f+M_h would require the least squares model to calculate smoothing parameter
            self.last_run_fopt += h(x0, *argsh)
    ...
    def add_new_direction_while_growing(self, number_of_samples, params, min_num_steps=0):
        ...
        for j in range(num_steps):
            xnew = xopt + (step_length / LA.norm(dirns[j, :])) * dirns[j, :]
            ...
            if self.model.npt() < self.model.num_pts:  # still growing
                ...
                kmin, linalg_error = self.choose_point_to_replace(xnew - xopt, skip_kopt=True)
    ...
    def geometry_step(self, knew, adelt, number_of_samples, params):
        ...
        if self.h != None:
            u = calculate_u(H, self.L_h, self.func_tol)
            obj += moreau_envelope(x, self.h, self.argsh, u, self.prox_uh, self.argsprox)
            # since m(0) = h(x)
    ...
    def evaluate_objective(self, x, number_of_samples, params):
        ...
        if self.h == None:
            ...
            if num_samples_run > 0 and \
                            sumsq(np.mean(rvec_list[:num_samples_run, :], axis=0)) + moreau_envelope(remove_scaling(x, self.scaling_changes), self.h, self.argsh, self.model.u, self.prox_uh, self.argsprox) <= self.model.min_objective_value():
                exit_info = ExitInformation(EXIT_SUCCESS, "Objective is sufficiently small")
    ...
    def terminate_from_slow_iterations(self, current_iter, params):
        if len(self.last_iters_step_taken) <= params("slow.history_for_slow"):
            ...
            if self.h != None:
                self.last_objopts_step_taken.append(self.model.est_objopt())
            else:
                self.last_objopts_step_taken.append(self.model.objopt())
        else:
            # Enough info - shift values
            self.last_iters_step_taken = self.last_iters_step_taken[1:] + [current_iter]
            if self.h != None:
                self.last_objopts_step_taken = self.last_objopts_step_taken[1:] + [self.model.est_objopt()]
                this_iter_slow = (log(self.last_objopts_step_taken[0]) - log(self.model.est_objopt())) / \
                             float(params("slow.history_for_slow")) < params("slow.thresh_for_slow")
            else:
                self.last_objopts_step_taken = self.last_objopts_step_taken[1:] + [self.model.objopt()]
                this_iter_slow = (log(self.last_objopts_step_taken[0]) - log(self.model.objopt())) / \
                             float(params("slow.history_for_slow")) < params("slow.thresh_for_slow")
    ...
    def soft_restart(self, number_of_samples, nruns_so_far, params, x_in_abs_coords_to_save=None, rvec_to_save=None,
        ...
        # A successful run is one where we reduced the observed minimum objective value
        if self.h != None:
            if self.model.est_objopt() < self.last_run_fopt:
                self.last_successful_run = nruns_so_far
            self.last_run_fopt = self.model.est_objopt()
        else:
        ...
        self.model.save_point(self.model.xopt(abs_coordinates=True), self.model.ropt(), self.model.nsamples[self.model.kopt], x_in_abs_coords=True)

        if self.do_logging:
            if self.h != None:
                # module_logger.info("Soft restart [currently, f = %g after %g function evals]" % (curr_objopt, self.nf))
    ...
    def all_values_within_noise_level(self, params):
        ...
        if params("noise.additive_noise_level") is not None:
            ...
            for k in range(self.model.npt()):
                if self.h != None:
                    all_fvals_within_noise = all_fvals_within_noise and \
                                            (self.model.est_objval[k] <= self.model.est_objopt() + add_noise / sqrt(self.model.nsamples[k]))
                else:
                    all_fvals_within_noise = all_fvals_within_noise and \
            ...
            for k in range(self.model.npt()):
                if self.h != None:
                    this_ratio = self.model.est_objval[k] / self.model.est_objopt() # objval_opt strictly positive (would have quit o/w)
                    all_fvals_within_noise = all_fvals_within_noise and \
    ...
    def reset_controller(self, func_tol_new):
        self.func_tol = func_tol_new

        # update model class parameters:
        # objective values dependent on smoothing parameter u
        self.model.reset_model(func_tol_new=func_tol_new)
        self.last_run_fopt = self.model.est_objopt()

        # forget history for slow iterations
        self.last_iters_step_taken = []
        self.last_objopts_step_taken = []
        self.num_slow_iters = 0
        
        # reset rhobeg as well?
