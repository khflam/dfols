...
from dfolssr.model import Model
from dfolssr.util import sumsq
...
class TestAddValues(unittest.TestCase):
    def runTest(self):
        ...
        self.assertAlmostEqual(model.objopt(), sumsq(rosenbrock(x1))+h(x1), None, 'Wrong objopt after x3')
...
class TestBasicManipulation(unittest.TestCase):
    def runTest(self):
        ...
        self.assertAlmostEqual(sqdists[0], sumsq(x0 - x1), None, 'Wrong distance 0')
        self.assertAlmostEqual(sqdists[1], sumsq(x1 - x1), None, 'Wrong distance 1')
        self.assertAlmostEqual(sqdists[2], sumsq(x2 - x1), None, 'Wrong distance 2')
        ...
        self.assertAlmostEqual(model.objopt(), sumsq(2 * rosenbrock(x0) / 52) + h(x0), None, 'Wrong objopt after bad resampling')
        ...
        x, rvec, obj, jacmin, nsamples = model.get_final_results()
        self.assertTrue(array_compare(x, x1), 'Wrong final x')
        self.assertTrue(array_compare(rvec, rosenbrock(x1)), 'Wrong final rvec')
        self.assertAlmostEqual(sumsq(rosenbrock(x1)) + h(x1), obj, None, 'Wrong final obj')
...
class TestMinObjValue(unittest.TestCase):
    def runTest(self):
        ...
        x2 = np.array([2.0, 0.9])
        model.change_point(2, x2 - model.xbase, rosenbrock(x2))
        self.assertAlmostEqual(model.min_objective_value(), 1e-12, None,'Wrong min obj value')
        model = Model(npt, x0, rosenbrock(x0), xl, xu, [], 1, h, rel_tol=1e-2)
        self.assertAlmostEqual(model.min_objective_value(), 1e-2 * (sumsq(rosenbrock(x0))+h(x0)), None, 'Wrong min obj value 2')
        model = Model(npt, x0, rosenbrock(x0), xl, xu, [], 1, h, abs_tol=1.0)
        self.assertAlmostEqual(model.min_objective_value(), 1.0, None, 'Wrong min obj value 3')
        model = Model(npt, x0, rosenbrock(x0), xl, xu, [], 1, h, abs_tol=1.0, rel_tol=1e-2)
        self.assertAlmostEqual(model.min_objective_value(), 1.0, None, 'Wrong min obj value 4')
...
class TestRegression(unittest.TestCase):
    def runTest(self):
        ...
        xu = 1e2 * np.ones((n,))
        h = lambda d: np.linalg.norm(d, 1)
        model = Model(npt, A[0,:], np.array([b[0]]), xl, xu, [], 1, h)
